...
# The parser is generated by [Jison](https://github.com/zaach/jison) from this grammar file. 
# The `kake parser` command constructs a parse table from our rules and saves it into `js/parser.js`.

{ Parser } = require 'jison'

# Jison DSL
# ---------
# Since we're going to be wrapped in a function by Jison in any case, if our action immediately returns a value, 
# we can optimize by removing the function wrapper and just returning the value directly.

unwrap = /^function\s*\(\)\s*\{\s*return\s*([\s\S]*);\s*\}/

# Our handy DSL for Jison grammar generation, thanks to [Tim Caswell](https://github.com/creationix). 
# For every rule in the grammar, we pass the pattern-defining string, the action to run, and extra options. 
...
# for the `UNLESS` terminal, and `$3` would be the value of the second `Expression`.
...
grammar =
    ...
    # The basic unit of Koffee is the **Expression** -- everything that can be an expression is one. 
    ...
    # Koffee has two different symbols for functions. `->` is for ordinary functions, and `=>` is for functions bound to the current value of *this*.
    ...
    # In Koffee, an object literal is simply a list of assignments.
    ...
    Export: [
        ...
        o 'EXPORT Identifier = Expression'                                                 -> new ExportNamedDeclaration new Assign $2, $4, null, moduleDeclaration: 'export'
        o 'EXPORT Identifier = TERMINATOR Expression'                                      -> new ExportNamedDeclaration new Assign $2, $5, null, moduleDeclaration: 'export'
        o 'EXPORT Identifier = INDENT Expression OUTDENT'                                  -> new ExportNamedDeclaration new Assign $2, $5, null, moduleDeclaration: 'export'
    ...
    # Note that the **Parenthetical** is a **Value**, not an **Expression**, so if you need to use an expression in a place
    # where only values are accepted, wrapping it in parentheses will always do the trick.
    ...
    # An array or range comprehension has variables for the current element and (optional) reference to the current index. 
    # Or, *key, value*, in the case of object comprehensions.
    ...
    Operation: [
        ...
        o 'SimpleAssignable COMPOUND_ASSIGN TERMINATOR Expression'                         -> new Assign $1, $4, $2
        o 'SimpleAssignable EXTENDS Expression'                                            -> new Extends $1, $3
    ]


#  0000000   00000000   00000000  00000000    0000000   000000000   0000000   00000000    0000000  
# 000   000  000   000  000       000   000  000   000     000     000   000  000   000  000       
# 000   000  00000000   0000000   0000000    000000000     000     000   000  0000000    0000000   
# 000   000  000        000       000   000  000   000     000     000   000  000   000       000  
#  0000000   000        00000000  000   000  000   000     000      0000000   000   000  0000000   

# Operators at the top of this list have higher precedence than the ones lower down.
# Following these rules is what makes `2 + 3 * 4` parse as: `2 + (3 * 4)` and not `(2 + 3) * 4`
