...
(function() {
    ...
    exports.MetaIf = MetaIf = (function(superClass1) {
        ...
        MetaIf.prototype.compileNode = function(o) {
            var args, bodyOpt, cond, err, frag, fs, indent, info, metaKey, os, ref10, ref11, ref12, ref3, ref4, ref5, ref6, ref7, ref8, ref9;
            info = {
                reduce: true
            };
            ...
            if (info["eval"] || (info.code == null)) {
                cond = (ref12 = info.code) != null ? ref12 : fragmentsToText(this.condition.compileToFragments(o, LEVEL_PAREN));
                try {
                    os = require('os');
                    fs = require('fs');
                    info.body = !!eval(cond);
                    if (info["eval"] && info.reduce && !info.body && !this.elseBody) {
                        return [];
                    }
                } catch (error) {
                    err = error;
            ...
            if (info.body) {
                frag = frag.concat(this.ensureBlock(this.body).compileToFragments(bodyOpt));
            }
            if (info.after) {
            ...
            if (this.elseBody && (info.reduce === false || info.body === false)) {
                if (!info.reduce) {
                    frag.push(this.makeCode(' else '));
                }
                if (this.isChain) {
                    frag = frag.concat(this.elseBody.unwrap().compileToFragments(bodyOpt));
                } else {
                    frag = frag.concat(this.elseBody.compileToFragments(bodyOpt));
                }
            }
            if (!frag.length) {
                frag.push(this.makeCode(''));
