...
FEATURES = [
    ['header',                    'prefix with "Generated by" header']
    ['config-parameters',         'enable config parameters']
    ['constructor-shortcut',      'replace @: with constructor:']
    ['negative-index',            'replace a[-1] with a[-1..-1]']
]
...
SWITCHES = [
    ...
    [      '--no-[feature]',      'disable a feature, e.g. --no-negative-index']
...
compilePath = (source, topLevel, base) ->
    ...
    if stats.isDirectory()
        ...
        try
            ...
            if err.code is 'ENOENT' then return else 
            throw err
...
parseOptions = ->
    ...
    # features  = ['constructor-shortcut', 'negative-index', 'config-parameters', 'header']
    abbrev    = ['bare', 'compile', 'help', 'inline-map', 'map', 'stdio', 'Debug'] # these can be merged, eg. -bcisD
    toggles   = abbrev.concat [
        'features', 'nodes', 'nodejs', 
        'print', 'tokens', 'no-rewrite', 'version', 'watch'
    ]
    ...
    # features.concat(toggles).map (n) -> known[n] = Boolean
    FEATURES.map((f) -> f[0]).concat(toggles).map (n) -> known[n] = Boolean
    
    short = M: ["--inline-map"]
    abbrev.map (a) -> short[a[0]] = ["--#{a}"]
...
# Start up a new Node instance with the arguments in `--nodejs` passed to the `node` binary, preserving the other options.
