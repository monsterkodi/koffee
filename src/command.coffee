# The `koffee` utility. Handles command-line compilation of Koffee
...
# Allow Koffee to emit Node.js events.
...
BANNER = '''
    Usage: koffee [options] path/to/script.coffee -- [args]
...
SWITCHES = [
    ...
    ['-i', '--interactive',         'run an interactive Koffee REPL']
...
# Watch a source Koffee file using `fs.watch`, recompiling it every
...
# The compile-time options to pass to the Koffee compiler.
...
version = ->
    printLine "koffee version #{Koffee.VERSION}"
