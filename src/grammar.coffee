# The Koffee parser is generated by [Jison](https://github.com/zaach/jison)
...
#
# If you run the `kake parser` command, Jison constructs a parse table from our rules and saves it into `lib/parser.js`.

{Parser} = require 'jison' # The only dependency is on the **Jison.Parser**.
...
grammar =
    ...
    # Koffee is the **Expression** -- everything that can be an expression
    ...
    # Koffee has two different symbols for functions. `->` is for ordinary
