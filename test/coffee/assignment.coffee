...
test "unassignable values", ->
    nonce = {}
    for nonref in ['', '""', '0', 'f()'].concat Koffee.RESERVED
        eq nonce, (try Koffee.compile "#{nonref} = v" catch e then nonce)
...
test "#1192: assignment starting with object literals", ->
    doesNotThrow (-> Koffee.run "{}.p = 0")
    doesNotThrow (-> Koffee.run "{}.p++")
    doesNotThrow (-> Koffee.run "{}[0] = 1")
    doesNotThrow (-> Koffee.run """{a: 1, 'b', "#{1}": 2}.p = 0""")
    doesNotThrow (-> Koffee.run "{a:{0:{}}}.a[0] = 0")
...
test "#1005: invalid identifiers allowed on LHS of destructuring assignment", ->
    disallowed = ['eval', 'arguments'].concat Koffee.RESERVED
...
test "#156: destructuring with expansion", ->
    ...
    eq 'x', last
    throws (-> Koffee.compile "[1, ..., 3]"),                null, "prohibit expansion outside of assignment"
    throws (-> Koffee.compile "[..., a, b...] = c"), null, "prohibit expansion and a splat"
    throws (-> Koffee.compile "[...] = c"),                  null, "prohibit lone expansion"
...
test "#1627: prohibit conditional assignment of undefined variables", ->
    throws (-> Koffee.compile "x ?= 10"),                null, "prohibit (x ?= 10)"
    throws (-> Koffee.compile "x ||= 10"),           null, "prohibit (x ||= 10)"
    throws (-> Koffee.compile "x or= 10"),           null, "prohibit (x or= 10)"
    throws (-> Koffee.compile "do -> x ?= 10"),  null, "prohibit (do -> x ?= 10)"
    throws (-> Koffee.compile "do -> x ||= 10"), null, "prohibit (do -> x ||= 10)"
    throws (-> Koffee.compile "do -> x or= 10"), null, "prohibit (do -> x or= 10)"
    doesNotThrow (-> Koffee.compile "x = null; x ?= 10"),                "allow (x = null; x ?= 10)"
    doesNotThrow (-> Koffee.compile "x = null; x ||= 10"),           "allow (x = null; x ||= 10)"
    doesNotThrow (-> Koffee.compile "x = null; x or= 10"),           "allow (x = null; x or= 10)"
    doesNotThrow (-> Koffee.compile "x = null; do -> x ?= 10"),  "allow (x = null; do -> x ?= 10)"
    doesNotThrow (-> Koffee.compile "x = null; do -> x ||= 10"), "allow (x = null; do -> x ||= 10)"
    doesNotThrow (-> Koffee.compile "x = null; do -> x or= 10"), "allow (x = null; do -> x or= 10)"
    ...
    for access in accesses
        for i,j in [1,2,3] #position can matter
            code =
                ...
                """
            eq nonce, unless (try Koffee.run code, bare: true catch e then true) then nonce
...
test '#2211: splats in destructured parameters', ->
    doesNotThrow -> Koffee.compile '([a...]) ->'
    doesNotThrow -> Koffee.compile '([a...],b) ->'
    doesNotThrow -> Koffee.compile '([a...],[b...]) ->'
    throws -> Koffee.compile '([a...,[a...]]) ->'
    doesNotThrow -> Koffee.compile '([a...,[b...]]) ->'

test '#2213: invocations within destructured parameters', ->
    throws -> Koffee.compile '([a()])->'
    throws -> Koffee.compile '([a:b()])->'
    throws -> Koffee.compile '([a:b.c()])->'
    throws -> Koffee.compile '({a()})->'
    throws -> Koffee.compile '({a:b()})->'
    throws -> Koffee.compile '({a:b.c()})->'
...
test "#1500: Assignment to variables similar to generated variables", ->
    ...
    doesNotThrow -> Koffee.compile '(@slice...) ->'
