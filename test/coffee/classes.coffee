...
test "#1534: class then 'use strict'", ->
    ...
    return unless (try Koffee.run strictTest, bare: yes catch e then nonce) is nonce

    throws -> Koffee.run "class then 'use strict';#{error}", bare: yes
    doesNotThrow -> Koffee.run "class then #{error}", bare: yes
    doesNotThrow -> Koffee.run "class then #{error};'use strict'", bare: yes
    ...
    throws (-> Koffee.run comment, bare: yes) for comment in comments
    ...
    throws (-> Koffee.run directive, bare: yes) for directive in directives
...
test "directives in class with extends ", ->
    ...
    """
    Koffee.run strictTest, bare: yes

test "#2630: class bodies can't reference arguments", ->
    throws ->
        Koffee.compile('class Test then arguments')
