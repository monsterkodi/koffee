...
cantCompile = (code) -> # assert that a string should fail compilation
    throws -> Koffee.compile code

test "ensure that carriage returns don't break compilation on Windows", ->
    doesNotThrow -> Koffee.compile 'one\r\ntwo', bare: on

test "#3089 - don't mutate passed in options to compile", ->
    opts = {}
    Koffee.compile '1 + 1', opts
    ok !opts.scope

test "--bare", ->
    eq -1, Koffee.compile('x = y', bare: on).indexOf 'function'
    ok 'passed' is Koffee.eval '"passed"', bare: on, filename: 'test'

test "header is disabled by default", ->
    header = "// generated by koffee #{Koffee.VERSION}\n"
    eq -1, Koffee.compile('x = y').indexOf header
...
test "reference named hasOwnProperty", ->
    Koffee.compile 'hasOwnProperty = 0; a = 1'
...
test "#2846: while with empty body", ->
    Koffee.compile 'while 1 then', {sourceMap: true}

test "#2944: implicit call with a regex argument", ->
    Koffee.compile 'o[key] /regex/'
