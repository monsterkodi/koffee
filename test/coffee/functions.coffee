...
test "@-parameters: automatically assign an argument's value to a property of the context", ->
    ...
    code = '((@prop) -> prop).call {}'
    doesNotThrow -> Koffee.compile code
    throws (-> Koffee.run code), ReferenceError
...
test "#156: parameter lists with expansion", ->
    ...
    throws (-> Koffee.compile "(..., a, b...) ->"), null, "prohibit expansion and a splat"
    throws (-> Koffee.compile "(...) ->"),                  null, "prohibit lone expansion"
...
test "default values with function calls", ->
    doesNotThrow -> Koffee.compile "(x = f()) ->"

test "arguments vs parameters", ->
    doesNotThrow -> Koffee.compile "f(x) ->"
    f = (g) -> g()
    eq 5, f (x) -> 5
...
test "#1038 Optimize trailing return statements", ->
    compile = (code) -> Koffee.compile(code, bare: yes).trim().replace(/\s+/g, " ")
