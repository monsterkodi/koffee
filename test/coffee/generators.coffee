...
test "error if `yield from` occurs outside of a function", ->
    throws -> Koffee.compile 'yield from 1'

test "`yield from` at the end of a function errors", ->
    throws -> Koffee.compile 'x = -> x = 1; yield from'
...
test "symbolic operators has precedence over the `yield`", ->
    ...
    for op in operators
        ...
        yielded = Koffee.eval "(arr) ->  yield #{expression} for i in arr"
        mapped  = Koffee.eval "(arr) ->             (#{expression} for i in arr)"
