...
test "#2060: Disallow uppercase radix prefixes and exponential notation", ->
    for char in ['b', 'o', 'x', 'e']
        program = "0#{char}0"
        doesNotThrow -> Koffee.compile program, bare: yes
        throws -> Koffee.compile program.toUpperCase(), bare: yes
...
test "Infinity", ->
    eq Infinity, Koffee.eval "0b#{Array(1024 + 1).join('1')}"
    eq Infinity, Koffee.eval "0o#{Array(342 + 1).join('7')}"
    eq Infinity, Koffee.eval "0x#{Array(256 + 1).join('f')}"
    eq Infinity, Koffee.eval Array(500 + 1).join('9')
    eq Infinity, 2e308
